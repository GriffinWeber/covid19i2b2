...
--******************************************************************************
--******************************************************************************
--*** Create temp tables with local lab and med code mappings
--******************************************************************************
--******************************************************************************
...
alter table #lab_mapping add primary key (loinc, local_lab_code)
insert into #lab_mapping
	select '6690-2','6690-2','1','10*3/uL','white blood cell count (Leukocytes)'
...
-- * First load the list of ATC and RxNorm codes into a temp table.
-- * Then convert to your local format.
-- * Do not change the med_class or add additional medications.
--------------------------------------------------------------------------------
create table #med_mapping (
	med_class varchar(50) not null,
	code_type varchar(10) not null,
	local_med_code varchar(50) not null
)
alter table #med_mapping add primary key (med_class, code_type, local_med_code)
-- Don't change this query. This defines the list of ATC and RxNorm codes for each 4CE medication class.
insert into #med_mapping
	select distinct m, t, c
	from (
		select m, 'ATC' t, c
		from (
...
-- * Modify the ICD codes to your local codes
--------------------------------------------------------------------------------
...
-- * Only run this if your data are accurate.
-- * Otherwise, use -999 for cumulative_patients_dead in the DailyCounts file.
--------------------------------------------------------------------------------
...
-- * Change the codes or the logic as needed for your site.
--------------------------------------------------------------------------------
...
insert into #Demographics_temp (patient_num, race)
	select patient_num,
		(case
			when race_cd in ('NA','AI','I') then 'american_indian'
			when race_cd in ('AS','A') then 'asian'
			when race_cd in ('B') then 'black'
			when race_cd in ('H','P') then 'hawaiian_pacific_islander'
			when race_cd in ('W') then 'white'
			else 'other' end) race
		from patient_dimension
		where patient_num in (select patient_num from #covid_cohort)
	union all
	select patient_num, 'hispanic_latino'
		from patient_dimension
		where patient_num in (select patient_num from #covid_cohort)
			and race_cd in ('HL')
...
--------------------------------------------------------------------------------
update #DailyCounts
	set cumulative_patients_all = cumulative_patients_all + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		cumulative_patients_severe = cumulative_patients_severe + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		cumulative_patients_dead = cumulative_patients_dead + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_in_hospital_on_this_date = num_patients_in_hospital_on_this_date + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_in_hospital_and_severe_on_this_date = num_patients_in_hospital_and_severe_on_this_date + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3
update #ClinicalCourse
	set num_patients_all_still_in_hospital = num_patients_all_still_in_hospital + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_ever_severe_still_in_hospital = num_patients_ever_severe_still_in_hospital + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3
update #Demographics
	set num_patients_all = num_patients_all + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_ever_severe = num_patients_ever_severe + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3
update #Labs
	set num_patients_all = num_patients_all + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_ever_severe = num_patients_ever_severe + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3
update #Diagnoses
	set num_patients_all_before_admission = num_patients_all_before_admission + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_all_since_admission = num_patients_all_since_admission + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_ever_severe_before_admission = num_patients_ever_severe_before_admission + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_ever_severe_since_admission = num_patients_ever_severe_since_admission + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3
update #Medications
	set num_patients_all_before_admission = num_patients_all_before_admission + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_all_since_admission = num_patients_all_since_admission + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
		num_patients_ever_severe_before_admission = num_patients_ever_severe_before_admission + FLOOR(ABS(BINARY_CHECKSUM(NEWID())/2147483648.0)*7) - 3,
...
--------------------------------------------------------------------------------
update #DailyCounts
	set cumulative_patients_all = (case when cumulative_patients_all<10 then -99 else cumulative_patients_all end),
		cumulative_patients_severe = (case when cumulative_patients_severe<10 then -99 else cumulative_patients_severe end),
		cumulative_patients_dead = (case when cumulative_patients_dead<10 then -99 else cumulative_patients_dead end),
		num_patients_in_hospital_on_this_date = (case when num_patients_in_hospital_on_this_date<10 then -99 else num_patients_in_hospital_on_this_date end),
		num_patients_in_hospital_and_severe_on_this_date = (case when num_patients_in_hospital_and_severe_on_this_date<10 then -99 else num_patients_in_hospital_and_severe_on_this_date end)
update #ClinicalCourse
	set num_patients_all_still_in_hospital = (case when num_patients_all_still_in_hospital<10 then -99 else num_patients_all_still_in_hospital end),
		num_patients_ever_severe_still_in_hospital = (case when num_patients_ever_severe_still_in_hospital<10 then -99 else num_patients_ever_severe_still_in_hospital end)
update #Demographics
	set num_patients_all = (case when num_patients_all<10 then -99 else num_patients_all end),
		num_patients_ever_severe = (case when num_patients_ever_severe<10 then -99 else num_patients_ever_severe end)
update #Labs
	set num_patients_all=-99, mean_value_all=-99, stdev_value_all=-99, mean_log_value_all=-99, stdev_log_value_all=-99
	where num_patients_all<10
update #Labs
	...
	where num_patients_ever_severe<10
update #Diagnoses
	set num_patients_all_before_admission = (case when num_patients_all_before_admission<10 then -99 else num_patients_all_before_admission end),
		num_patients_all_since_admission = (case when num_patients_all_since_admission<10 then -99 else num_patients_all_since_admission end),
		num_patients_ever_severe_before_admission = (case when num_patients_ever_severe_before_admission<10 then -99 else num_patients_ever_severe_before_admission end),
...
-- Delete small counts.
--------------------------------------------------------------------------------
delete from #DailyCounts where cumulative_patients_all<10
delete from #ClinicalCourse where num_patients_all_still_in_hospital<10
delete from #Labs where num_patients_all<10
delete from #Diagnoses where num_patients_all_before_admission<10 and num_patients_all_since_admission<10
delete from #Medications where num_patients_all_before_admission<10 and num_patients_all_since_admission<10
